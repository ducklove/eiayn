...
규모 점수의 입력을 원통화 `aum`에서 공통 통화 `aumUsd`로 변경했습니다. 동일한 관측일의 환율로 환산한 후 로그 정규화하며, 환산할 수 없는 외화 금액을 원금액으로 대체하지 않습니다. 가중치와 다른 점수 구성요소의 정규화 방식은 유지합니다. 동점 순위도 충족도 → USD 규모 → 코드 순입니다.

`scoreComponents`는 실제 점수 계산에 쓰인 일곱 구성요소를 보존해, 레이더의 합성 팩터에서 가려질 수 있는 누락 요소를 표시합니다. `scoreCoverage`는 계산 가능한 구성요소의 가중치 합입니다. 한 구성요소 안의 세부 지표가 모두 있다는 뜻은 아니며, 예를 들어 추적오차 없이 정보비율만 있어도 추종 안정성 구성요소를 계산할 수 있습니다.

시장·자산군·최소 충족도를 선택하면 해당 집단 안의 순위를 표시하지만 점수 자체는 전체 유니버스 기준입니다. 자산군 내 백분위 점수로 재계산한 값이 아닙니다.

스냅샷과 일별 이력에 산식 버전을 보존하고 동일한 버전의 점수만 추이로 연결합니다. 과거 산식의 점수를 덮어쓰지 않으며, 버전 전환을 점수 급변으로 알리지 않습니다. 환율 변화·유니버스 구성·데이터 확보 변화도 점수에 영향을 줄 수 있으므로 점수 변화가 투자 성과 변화와 동일하지 않습니다.
