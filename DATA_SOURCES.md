...
| Source | URL | Fields |
| --- | --- | --- |
| Naver Finance ETF 전종목 시세 | `https://finance.naver.com/api/sise/etfItemList.nhn` | Korea ETF lineup, price, 1-day return, live iNAV, volume, trading value (백만원), market cap (억원) — one request covers the full universe (EUC-KR response) |
| Naver Finance ETF 분석 | `https://m.stock.naver.com/api/stock/{code}/etfAnalysis` | per-ETF issuer, total fee, base index, listing date, official previous-day NAV, premium/discount (괴리율), 3M/1Y returns, TTM dividend yield, theme labels, top-10 holdings |
...
| ~~K-ETF~~ (retired 2026-07) | `https://anchor.k-etf.com/api/` | former Korea source; the API was privatized behind K-ETF's own backend and returns 403 to all external callers since 2026-06-26 |
| ~~KRX 정보데이터시스템~~ (retired 2026-07) | `http://data.krx.co.kr/` | former NAV/괴리율 source; relaunched as the login-only "KRX Data Marketplace", so the anonymous batch endpoint is gone |
...
Since 2026-07 (round: K-ETF/KRX source outage), Korean ETF coverage starts from the Naver Finance full-lineup quote endpoint:

```text
https://finance.naver.com/api/sise/etfItemList.nhn?etfType=0&targetColumn=market_sum&sortOrder=desc
```

One request returns every listed Korean ETF (`result.etfItemList`, EUC-KR encoded — the fetch layer decodes the charset explicitly) with `itemcode`, `itemname`, price (`nowVal`), 1-day change (`changeRate`), live iNAV (`nav`), volume (`quant`), trading value in 백만원 (`amonut`), and market cap in 억원 (`marketSum`); trading value and market cap are rescaled to raw KRW for the snapshot. KRX short codes are six alphanumeric characters (the post-2026 scheme includes letters, e.g. `0167A0`).

The lineup request is required: no lineup, no Korean universe, and the refresh fails without committing. Records with no usable positive price are excluded, logged in `coverage.excluded`, and counted against `coverage.korea.sourceTotal` during validation so one incomplete listing cannot block the whole daily refresh.

Each ETF is then enriched from the per-ETF analysis endpoint (strictly best-effort — a failed fetch leaves the profile fields `null` and holdings empty, never excludes the ETF):

```text
https://m.stock.naver.com/api/stock/{code}/etfAnalysis
```

It supplies `issuerName` (the `(ETF)` suffix is stripped), `totalFee` (expense ratio, %), `etfBaseIndex` (benchmark display name), `listedDate` (inception), the official previous-day `nav` plus signed `deviationRate` (괴리율 → `premiumDiscount`, published only when |x| ≤ 50), `returnPerformanceList` `M3`/`Y1` returns, `dividend.dividendYieldTtm`, Naver theme labels, and `etfTop10MajorConstituentAssets` (top-10 holdings; rows whose weight Naver reports as `-` — common for foreign-listed and bond constituents — are kept with `weight: null` so their names stay searchable, and the scorer counts only weighted rows toward concentration so a weightless row never reads as zero concentration). `dataQuality.navAsOf` records `navPerformanceReferenceDate` when NAV is present. Asset class and theme are derived from the ETF name, base index, and Naver theme labels by `classifyKoreanAssetClass` / `classifyKoreanTheme` in `scripts/data/naver.mjs`, reproducing the established snapshot vocabulary (주식/채권/혼합자산/원자재/ETF; 시장대표/배당/커버드콜/...).

### Yahoo KRX long-horizon enrichment

Naver supplies point-in-time quotes and 3M/1Y returns but no usable long history, so after the Naver build each Korean ETF is optionally enriched from the Yahoo Finance chart for its KRX symbol (`${code}.KS`, 5-year range). Naver values always keep priority: the enrichment fills only fields that are still `null` after the Naver build.
...
- `returns.m3`, `returns.y1`, `dividendYield` as fallbacks when Naver left them empty, and `sparkline`/`performance1y`, which the Naver build always leaves empty.
...
`nav` and `premiumDiscount` come from the Naver analysis endpoint (official previous-trading-day NAV and signed 괴리율) instead of the retired KRX batch table. The snapshot-wide invariant is unchanged: `premiumDiscount: null` exists on every ETF, `nav` must be null-or-positive, and `premiumDiscount` must be null or a finite percentage within ±50 (`npm run check:data` enforces both). Values are parsed defensively and never estimated; implausible deviations degrade to `null`.
...
- Optional StockAnalysis, Yahoo quoteSummary, Naver per-ETF analysis, and holdings failures are logged and leave affected fields missing unless `check:data` marks the field as required.
...
- Korean Yahoo KRX enrichment is optional per ETF: a failed or empty chart leaves that ETF exactly as the Naver build produced it, never excludes it, and never aborts the run. The update log reports a single summary line with enriched/unavailable counts.
...
- Korean price history comes entirely from the best-effort Yahoo KRX enrichment; all series-derived fields (sparkline, `performance1y`, 3-year/5-year return/risk) remain `null`/empty for ETFs where Yahoo has no usable `.KS` history (delistings, very recent listings, new-scheme alphanumeric codes Yahoo does not cover) or where the enrichment fetch fails.
- Korean holdings are Naver's top-10 constituents (down from up to 25 in the K-ETF era). ETFs whose constituent weights Naver reports as `-` (mostly foreign-listed underlyings and bonds) keep name-only holdings rows: search still works, the portfolio card shows `-` weights, and the diversification score component is `null`.
- `nav` and `premiumDiscount` come from the Naver analysis endpoint and exist only for Korean ETFs; non-Korean ETFs keep `null` because no equivalent batched public NAV source is wired yet. `risk.trackingError3y` and `risk.informationRatio3y` are computed only for ETFs whose `benchmarkIndex` maps to a known Yahoo index symbol; the rest stay `null`. Naver base-index names are display strings (e.g. `코스피 200`, `S&P 500 Total Return Index`), and the mapping table carries explicit aliases for the common ones.
- Naver Finance endpoints are public web endpoints, not guaranteed official APIs; the K-ETF and KRX 정보데이터시스템 precedents show such sources can disappear without notice.
