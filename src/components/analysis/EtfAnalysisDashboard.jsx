...
const FACTOR_DESCRIPTIONS = {
  ...
  효율성:
    '낮은 총보수와 추적 안정성(추적오차·정보비율)을 함께 반영합니다. 추적 데이터가 없으면 총보수 중심으로 계산됩니다.',
};
  ...
  return (
    <section className="single-etf-dashboard" id="model" aria-labelledby="single-analysis-title">
      ...
      <div className="metric-tile-grid">
        <MetricTile
          label="총보수 (연)"
          value={formatPlainPercent(selectedEtf.expenseRatio)}
          tone="cost"
        />
        ...
        <MetricTile
          label="3개월 수익률"
          value={formatPercent(selectedEtf.returns.m3)}
          tone={m3Tone}
        />
        <MetricTile
          label="1년 수익률"
          value={formatPercent(selectedEtf.returns.y1)}
          tone={y1Tone}
        />
        <MetricTile
          label="3년 연환산"
          value={formatPercent(selectedEtf.returns.y3Annualized)}
          tone={y3Tone}
        />
        <MetricTile
          label="5년 연환산"
          value={formatPercent(selectedEtf.returns.y5Annualized)}
          tone={y5Tone}
        />
      </div>
          ...
          <div className="section-heading">
            <div className="heading-title">
              ...
              <InfoPopover title="AIYN 팩터란?">
                <p>
                  AIYN 점수는 비용, 규모, 단기 수익, 장기 수익, 위험조정, 추종 안정성, 분산도를
                  0-100으로 정규화한 뒤 데이터가 있는 항목끼리 가중 평균합니다.
                </p>
                <p>
                  데이터가 없는 팩터는 0점 처리하지 않고 계산에서 제외한 뒤 남은 가중치를
                  재배분합니다.
                </p>
              </InfoPopover>
            </div>
          ...
          <div className="factor-list">
            {factorEntries.map(([label, value]) => (
              <details className="factor-row" key={label}>
                ...
                <p>
                  {FACTOR_DESCRIPTIONS[label] ??
                    '해당 팩터는 수집된 실제 지표를 전체 ETF 유니버스 기준으로 정규화한 값입니다.'}
                </p>
              </details>
            ))}
          </div>
      ...
      <button
        className={`floating-favorite ${favorites.includes(selectedEtf.id) ? 'selected' : ''}`}
        type="button"
